...
async def plugin_install(message: "Message"):
...
async def plugin_remove(message: "Message"):
...
async def plugin_status(message: "Message"):
    if len(message.parameter) == 1:
        (
            active_plugins,
            disabled_plugins,
        ...
        await message.edit(output, parse_mode="md")
    else:
        await message.edit(lang("arg_error"))
...
async def plugin_enable(message: "Message"):
    if len(message.parameter) == 2:
...
async def plugin_disable(message: "Message"):
    if len(message.parameter) == 2:
...
async def plugin_upload(message: "Message"):
        ...
        if exists(file_name):
            try:
                ...
                await upload_attachment(
                    ...
                    f"PagerMaid-Modify {message.parameter[1]} plugin.",
                    parse_mode="html",
                )
...
async def plugin_update(message: "Message"):
    plugin_directory = f"{working_dir}{sep}plugins{sep}"
...
async def plugin_search(message: "Message"):
    if len(message.parameter) == 1:
...
async def plugin_show(message: "Message"):
    if len(message.parameter) == 1:
        await message.edit(lang("apt_search_no_name"))
    elif len(message.parameter) == 2:
        await plugin_manager.load_remote_plugins()
...
async def plugin_list(message: "Message"):
    await plugin_manager.load_remote_plugins()
...
async def plugin_export(message: "Message"):
    plugin_directory = f"{working_dir}{sep}plugins{sep}"
    if not exists(f"{plugin_directory}version.json"):
        await message.edit(lang("apt_why_not_install_a_plugin"))
        return
