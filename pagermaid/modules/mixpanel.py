...
async def set_people(bot: Client, force_update: bool = False):
    ...
    data = {"$first_name": bot.me.first_name, "is_premium": bot.me.premium}
    if bot.me.username:
...
async def mixpanel_report(bot: Client, message: Message, command, sub_command):
    ...
    if sender_id < 0 and message.out:
        sender_id = bot.me.id
...
async def get_sponsored(
    ...
    result = await bot(GetSponsoredMessagesRequest(peer=channel))
...
async def read_sponsored(
    ...
    result = await bot(ViewSponsoredMessageRequest(random_id=random_id))
    if result:
        bot.loop.create_task(
            mp.track(
                str(bot.me.id),
...
async def click_sponsored(
    ...
    result = await bot(ClickSponsoredMessageRequest(random_id=random_id))
    if result:
        bot.loop.create_task(
            mp.track(
                str(bot.me.id),
...
async def log_sponsored_clicked_one(username: str):
    ...
    channel = await userbot.get_input_entity(entity)
...
async def log_sponsored_clicked():
    ...
    if (not userbot.me) or userbot.me.premium:
        return
